...
test('record-shaped sections are reported as not stored, never silently discarded', () => {
  ...
  //
  // ⚠ `workouts` LEFT this list on 5 Sep 2026 — it has its own table and parser
  // now (`parseWorkouts`, `health_workouts`), because it was the one
  // record-shaped section with a consumer already waiting for it: leaving it
  // counted-but-discarded meant paying for a Strava OAuth integration to fetch
  // what the phone had been sending all along. Pinned by
  // apple-health-workouts.test.js.
  assert.deepEqual(out.unstored, { ecg_recordings: 1, audiograms: 2 });
